const timeTable=[{num:1,start:"08:30",end:"09:10"},{num:2,start:"09:20",end:"10:00"},{num:3,start:"10:20",end:"11:00"},{num:4,start:"11:10",end:"11:50"},{num:5,start:"12:10",end:"12:50"},{num:6,start:"13:10",end:"13:50"},{num:7,start:"14:00",end:"14:40"},{num:8,start:"14:50",end:"15:30"}];
const daysData={
...
let startX=0,startY=0,currentTranslate=0,prevTranslate=0,isDragging=false,currentIdx=0,dragDirection=null,lastHeartbeat=Date.now(),touchX=null,touchY=null,activePalette=null,targetTouchX=null,targetTouchY=null,currentTouchX=null,currentTouchY=null;
...
function selectRandomPalette(){
    const isDark=document.documentElement.getAttribute('data-theme')==='dark';activePalette=(isDark?darkPalettes:lightPalettes)[Math.floor(Math.random()*3)];
...
function generateFluidBackground(){
    if(!activePalette)selectRandomPalette();const isDark=document.documentElement.getAttribute('data-theme')==='dark';
    ...
    if(currentTouchX!==null&&currentTouchY!==null){positions.push({x:currentTouchX*1.2,y:currentTouchY*1.2,isTouch:true});}
    positions.forEach((pos,index)=>{
        ...
        let radialGrad=ctx.createRadialGradient(targetX,targetY,0,targetX,targetY,radius);let color=activePalette.colors[index%activePalette.colors.length];
        radialGrad.addColorStop(0,color+(isDark?'88':'ff'));radialGrad.addColorStop(0.5,color+(isDark?'11':'33'));radialGrad.addColorStop(1,'transparent');
...
function renderLoop(){
    if(targetTouchX!==null&&targetTouchY!==null){if(currentTouchX===null){currentTouchX=targetTouchX;currentTouchY=targetTouchY;}currentTouchX+=(targetTouchX-currentTouchX)*0.1;currentTouchY+=(targetTouchY-currentTouchY)*0.1;}
    else{if(currentTouchX!==null){currentTouchX+=(null-currentTouchX)*0.1;currentTouchY+=(null-currentTouchY)*0.1;if(Math.abs(currentTouchX)<1)currentTouchX=currentTouchY=null;}}
    generateFluidBackground();requestAnimationFrame(renderLoop);
}
window.addEventListener('touchstart',e=>{if(e.target.closest('.navigation-tabs')||e.target.closest('.lessons-list'))return;const t=e.touches[0];startX=t.clientX;startY=t.clientY;targetTouchX=t.clientX;targetTouchY=t.clientY;isDragging=true;dragDirection=null;});
window.addEventListener('touchmove',e=>{
    if(!isDragging)return;const t=e.touches[0];let diffX=t.clientX-startX,diffY=t.clientY-startY;targetTouchX=t.clientX;targetTouchY=t.clientY;
    if(!dragDirection){if(Math.abs(diffX)>Math.abs(diffY)+10)dragDirection='horizontal';else if(diffY>10&&currentIdx===0)dragDirection='pull';}
    if(dragDirection==='horizontal'){currentTranslate=prevTranslate+diffX;swiper.style.transform=`translateX(${currentTranslate}px)`;}
    else if(dragDirection==='pull'){let p=Math.min(diffY*0.4,90);pullIndicator.style.transform=`translate3d(-50%,${p}px,0)`;pullIndicator.style.opacity=Math.min(p/60,1);pullSvg.style.transform=`rotate(${p*4}deg)`;}
});
window.addEventListener('touchend',()=>{
    if(!isDragging)return;isDragging=false;
    if(dragDirection==='horizontal'){let m=currentTranslate-prevTranslate;if(m<-100&&currentIdx<1)currentIdx++;if(m>100&&currentIdx>0)currentIdx--;switchScreen(currentIdx);}
...
window.addEventListener('click',e=>{if(e.target.closest('.navigation-tabs')||e.target.closest('.lessons-list'))return;selectRandomPalette();});
function switchScreen(index){
    currentIdx=index;currentTranslate=currentIdx*-window.innerWidth;prevTranslate=currentTranslate;
    const sDay=document.getElementById('slide-day'),sWeek=document.getElementById('slide-week'),eff=Math.floor(Math.random()*3)+1;
...
window.addEventListener('deviceorientation',e=>{
    if(!e.gamma||!e.beta)return;let x=Math.min(Math.max(e.gamma,-30),30)/1.5,y=Math.min(Math.max(e.beta-45,-30),30)/1.5;
    canvas.style.transform=`translate3d(${x*1.1}px,${y*1.1}px,0) scale(1.15)`;document.querySelectorAll('.gyro-tilt').forEach(el=>el.style.transform=`rotateY(${x/2}deg) rotateX(${-y/2}deg) translateZ(15px)`);
});
...
function updateLogic(){
    ...
    for(let slot=1;slot<=8;slot++){
        ...
        const row=document.createElement('div');row.className=`lesson-row ${activeLessonId===slot?'active':''}`;const currentSlotTime=timeTable.find(t=>t.num===slot);
